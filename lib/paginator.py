...
class SQLAlchemyPaginator(Paginator):
    def __init__(self, query, per_page, table=None, orphans=0, allow_empty_first_page=True, query_count = None):
        self.query = query
        self.per_page = int(per_page)
        self.table = table
        self.orphans = int(orphans)
    ...
    def _get_count(self):
        ...
        if self._count is None:
            if self.query_count:
                self._count = self.query_count.scalar()
            elif self.table:
                self._count = self.query.with_entities(func.count(self.table.id)).first()[0]
            else:
...
class SQLAlchemyPage(Page):
    ...
    def __len__(self):
        if self.paginator.table:
            return self.paginator.query.with_entities(func.count(self.paginator.table.id)).first()[0]
        else:
